...
exports.insertAdvanceEntries = async (reportId, entries) => {
  for (const entry of entries) {
    await db.query(
      'INSERT INTO advance_entries (report_id, name, amount) VALUES (?, ?, ?)',
...
exports.updateTotalAdvance = async (reportId) => {
  const [rows] = await db.query(
    'SELECT SUM(amount) AS total FROM advance_entries WHERE report_id = ?',
    [reportId]
  );
  const total = rows[0].total || 0;

  await db.query(
    'UPDATE daily_reports SET advance = ? WHERE id = ?',
    [total, reportId]
  );
